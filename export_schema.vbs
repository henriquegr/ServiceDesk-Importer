...
strLogPath = strPath   
iTimeOut = 120
Set ocmds = fso.OpenTextFile(strPath & "\logs\cmdse.log", 8, True)

If fso.FileExists(strNxroot & "\site\mods\wsp.altercol") then
    copyFiles
    msgbox "Esquema de banco de dados e objeto salvo com sucesso.", 1, "Operacao efetuada com sucesso"        
else
    msgbox "O arquivo wsp.altercol nao foi encontrado. Por favor, publique o modelo de objeto no wsp antes de efetuar a migracao.", 0, "ERRO"
end if 
...
Function runCmdwLog(cmd, logfile)
    ...
    hora = retornaData
    errLog = strPath & "\logs\" &  re.replace(logfile, "_") & ".err"
    exeLog = strPath & "\logs\" &  re.replace(logfile, "_") & ".log"
